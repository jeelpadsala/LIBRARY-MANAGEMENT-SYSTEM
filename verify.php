...
if(isset($_POST['verify'])) {
    $otp = $_POST['otp'];
    $checkOtpSql = "SELECT StudentId FROM tblstudents WHERE otp = :otp AND Status = 0";
    $checkOtpQuery = $dbh->prepare($checkOtpSql);
    $checkOtpQuery->bindParam(':otp', $otp, PDO::PARAM_STR);
    $checkOtpQuery->execute();
    $studentId = $checkOtpQuery->fetch(PDO::FETCH_ASSOC);
    if($studentId) {
        $updateStatusSql = "UPDATE tblstudents SET Status = 1 WHERE StudentId = :studentId";
        $updateStatusQuery = $dbh->prepare($updateStatusSql);
        $updateStatusQuery->bindParam(':studentId', $studentId['StudentId'], PDO::PARAM_STR);
        $updateStatusQuery->execute();
        echo '<script>alert("OTP VERIFIED SUCCESSFULLY , NOW YOUR ACCOUNT IS ACTIVATED");</script>';
